...
public class Dinamit_Zone : EnvirInterBoyGirl_Class
{
    ...
    public override void EnvirWorck()
    {
        ...
        else if (isBoy == true && girlOn == true)
        {
            scaneData._BoyAnimator.SetBool("IsGirlRunEvent", true);
            scaneData.dinamitOn[0] = true;
            scaneData.Dinamit_01On();
    ...
    {
        if (scaneData.dinamitOn[0] == false)
        {
        ...
        else if (scaneData.dinamitOn[0] == true)
        {
            infoButRef.SetActive(false);
        }
    }
